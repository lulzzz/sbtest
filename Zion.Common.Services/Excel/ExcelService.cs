	...
	public class ExcelService : BaseService, IExcelService
	{
		...
		public FileDto GetEmployeeImportTemplate(Guid companyId)
		{
			try
			{
				...
				employees.OrderBy(e=>e.CompanyEmployeeNo).ToList().ForEach(e =>
				{
					var row = new List<string>
					{
						...
						e.Contact.Mobile,
						e.Contact.Fax,
						e.Contact.Address.AddressLine1,
						e.Contact.Address.City,
						"CA",
						e.Contact.Address.Zip,
						e.Contact.Address.ZipExtension,
						e.Gender.GetDbName(),
					...
					row.Add(e.State.State.Abbreviation);
					row.Add(((int)e.State.TaxStatus).ToString());
					row.Add(e.State.Exemptions.ToString());
					row.Add(e.State.AdditionalAmount.ToString());

					rows.Add(row);
				});


				return _excelRepository.GetImportTemplate(company.Name + "_EmployeeImport.xlsx", columnList, rows, true);
			}
			catch (Exception e)
			{
