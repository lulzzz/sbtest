update employee set PayCodes='[{"Id":-1,"CompanyId":"' + cast(CompanyId as varchar(max)) + '","Code":"PieceRate","Description":"Piece-work","HourlyRate":0.0}, {"Id":0,"CompanyId":"' + cast(CompanyId as varchar(max)) + '","Code":"Default","Description":"Base Rate","HourlyRate":' + (select cast(MinWage as varchar(max)) from Company where Id=employee.CompanyId) + '}]'
where paytype=3

/****** Object:  StoredProcedure [dbo].[GetExtractData]    Script Date: 17/01/2017 10:59:22 PM ******/
	...
	(
		select 
			...
			(
				...
				and not exists (select ''x'' from PayrollInvoice where id=PayrollPayCheck.InvoiceId and StatusId=5)
