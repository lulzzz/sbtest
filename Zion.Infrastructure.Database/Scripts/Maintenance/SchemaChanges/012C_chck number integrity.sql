/****** Object:  StoredProcedure [dbo].[EnsureCheckNumberIntegrity]    Script Date: 25/04/2018 1:45:23 PM ******/
...
GO
/****** Object:  UserDefinedFunction [dbo].[GetCheckNumber]    Script Date: 25/04/2018 1:45:23 PM ******/
	...
	select @result = case
		when @TransactionType=1 then
			case when @PEOASOCoCheck=1 and exists(select ''x'' from dbo.CompanyJournal where CheckNumber=@CheckNumber and PayrollPayCheckId<>@PayrollPayCheckId) then
				...
				when @PEOASOCoCheck=0 and @IsPEOPayroll=0 and exists(select ''x'' from dbo.CompanyPayCheckNumber where CompanyIntId=@CompanyId and Id<>@PayrollPayCheckId and CheckNumber=@CheckNumber) then
