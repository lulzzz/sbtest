<xsl:stylesheet version="1.0"
 ...
 <Worksheet ss:Name="777777777">
  <Table x:FullColumns="1"
   ...
   <Row></Row>
		<xsl:apply-templates select="Hosts/ExtractHost[count(PayCheckAccumulation/PayCheckList/PayCheckSummary)>0]" mode="i777">
			<xsl:sort select="HostCompany/TaxFilingName"/>
...
<Cell><Data ss:Type="Number"><xsl:choose><xsl:when test="PayCheckAccumulation/DailyAccumulations/DailyAccumulation[Month=3 and Day=31]"><xsl:value-of select="format-number(PayCheckAccumulation/DailyAccumulations/DailyAccumulation[Month=3 and Day=31]/Value,'#,###,##0.00')"/></xsl:when><xsl:otherwise>0</xsl:otherwise></xsl:choose></Data></Cell>
	...
	<xsl:template match="ExtractHost" mode="i777">
		...
		<xsl:variable name="totalTips" select="sum(PayCheckAccumulation/Compensations/PayCheckCompensation[PayTypeId=3]/YTD)"/>
		<xsl:variable name="fitWages" select="PayCheckAccumulation/Taxes/PayCheckTax[Tax/Code='FIT']/YTDWage"/>
		<xsl:variable name="totalSSWages" select="PayCheckAccumulation/Taxes/PayCheckTax[Tax/Code='SS_Employee']/YTDWage"/>
