...
<xsl:call-template name="padRight"><xsl:with-param name="data" select="$accountNumber"/><xsl:with-param name="count" select="10"/></xsl:call-template><xsl:value-of select="$amount"/>$$spaces5$$<xsl:call-template name="padRight"><xsl:with-param name="data" select="$checkNumber"/><xsl:with-param name="count" select="13"/></xsl:call-template>$$spaces5$$$$spaces2$$<xsl:choose><xsl:when test="IsReIssued='true'"><xsl:value-of select="msxsl:format-date(ReIssuedDate, 'yyyyMMdd')"/></xsl:when><xsl:otherwise><xsl:value-of select="$issueDate"/></xsl:otherwise></xsl:choose><xsl:choose>
	<xsl:when test="IsVoid='true'">$$spaces5$$$$spaces2$$$$spaces1$$V</xsl:when>
	<xsl:otherwise>$$spaces5$$$$spaces2$$$$spaces2$$</xsl:otherwise>
...
</xsl:call-template>$$spaces5$$$$spaces2$$<xsl:value-of select="msxsl:format-date(TransactionDate, 'yyyyMMdd')"/>$$spaces5$$$$spaces2$$$$spaces1$$V<xsl:text>$$n</xsl:text>
</xsl:when>
</xsl:choose>
</xsl:template>
