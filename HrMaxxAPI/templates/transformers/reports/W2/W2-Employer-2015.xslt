...
<xsl:param name="selectedYear"/>
  ...
  <xsl:variable name="uppercase" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ'"/>
	...
	<xsl:variable name="compStateDetails" select="concat('CA',' ', substring(/ReportResponse/Company/States[CompanyTaxState/State/StateId=1]/CompanyTaxState/StateEIN, 1, 3), '-', substring(/ReportResponse/Company/States[CompanyTaxState/State/StateId=1]/CompanyTaxState/StateEIN, 4, 4), '-', substring(/ReportResponse/Company/States[CompanyTaxState/State/StateId=1]/CompanyTaxState/StateEIN, 8, 1))"/>
	<xsl:variable name="empCount" select="count(/ReportResponse/EmployeeAccumulations/EmployeeAccumulation)"/>
...
<xsl:template name="Report">
	...
	<Report>
		...
		<Fields>
			...
			<xsl:if test="/ReportResponse/EmployeeAccumulations/EmployeeAccumulation[position()=$starter]">
				<xsl:call-template name="companySection">
					<xsl:with-param name="empNum" select="$starter mod 3"/>
				</xsl:call-template>
				<xsl:apply-templates select="/ReportResponse/EmployeeAccumulations/EmployeeAccumulation[position()=$starter]">
					<xsl:with-param name="index" select="$starter"/>
				</xsl:apply-templates>		
			</xsl:if>
			<xsl:if test="/ReportResponse/EmployeeAccumulations/EmployeeAccumulation[position()=$starter+1]">
				<xsl:call-template name="companySection">
					<xsl:with-param name="empNum" select="($starter+1) mod 3"/>
				</xsl:call-template>
				<xsl:apply-templates select="/ReportResponse/EmployeeAccumulations/EmployeeAccumulation[position()=$starter+1]">
					<xsl:with-param name="index" select="$starter+1"/>
				</xsl:apply-templates>		
			</xsl:if>
			<xsl:if test="/ReportResponse/EmployeeAccumulations/EmployeeAccumulation[position()=$starter+2]">
				<xsl:call-template name="companySection">
					<xsl:with-param name="empNum" select="($starter+2) mod 3"/>
				</xsl:call-template>
				<xsl:apply-templates select="/ReportResponse/EmployeeAccumulations/EmployeeAccumulation[position()=$starter+2]">
...
<xsl:template match="EmployeeAccumulation">
	...
	<xsl:variable name="FITWage" select="format-number(Accumulation/Taxes/PayrollTax[Tax/Code='FIT']/TaxableWage,'###0.00')"/>
	<xsl:variable name="FITTax" select="format-number(Accumulation/Taxes/PayrollTax[Tax/Code='FIT']/Amount,'###0.00')"/>
	<xsl:variable name="SSWage" select="format-number(Accumulation/Taxes/PayrollTax[Tax/Code='SS_Employee']/TaxableWage,'###0.00')"/>
	<xsl:variable name="SSTax" select="format-number(Accumulation/Taxes/PayrollTax[Tax/Code='SS_Employee']/Amount,'###0.00')"/>
	<xsl:variable name="MDWage" select="format-number(Accumulation/Taxes/PayrollTax[Tax/Code='MD_Employee']/TaxableWage,'###0.00')"/>
	<xsl:variable name="MDTax" select="format-number(Accumulation/Taxes/PayrollTax[Tax/Code='MD_Employee']/Amount,'###0.00')"/>
	<xsl:variable name="Tips" select="format-number(sum(Accumulation/Compensations[PayType/Id=3]/Amount),'###0.00')"/>
	<xsl:variable name="SITWage" select="format-number(Accumulation/Taxes/PayrollTax[Tax/Code='SIT']/TaxableWage,'###0.00')"/>
	<xsl:variable name="SITTax" select="format-number(Accumulation/Taxes/PayrollTax[Tax/Code='SIT']/Amount,'###0.00')"/>
	<xsl:variable name="SDIWage" select="format-number(Accumulation/Taxes/PayrollTax[Tax/Code='SDI']/TaxableWage,'###0.00')"/>
	<xsl:variable name="SDITax" select="format-number(Accumulation/Taxes/PayrollTax[Tax/Code='SDI']/Amount,'###0.00')"/>
...
<xsl:template match="PayrollDeduction">
	...
	<xsl:if test="position()=1">
		<xsl:call-template name="W2Repeater">
			...
			<xsl:with-param name="val" select="concat(Deduction/Type/W2_12,' ',format-number(Amount,'###0.00'))"/>
