...
<xsl:variable name="c" select="format-number(/ReportResponse/CompanyAccumulations/PayCheckWages/EmployeeCount,'#####0')"/>
...
<xsl:template match="Company">	
	<Report>
		...
		<Template>HW-3.pdf</Template>
		<Fields>
      <xsl:call-template name="eachLetter">
        <xsl:with-param name="startIndex" select="1"/>
        <xsl:with-param name="prefix" select="'y-'"/>
        <xsl:with-param name="data" select="$selectedYear"/>
      </xsl:call-template>
			<xsl:call-template name="eachLetter">
        <xsl:with-param name="startIndex" select="1"/>
      ...
      <xsl:call-template name="eachLetterReverse">
        <xsl:with-param name="startIndex" select="6"/>
        <xsl:with-param name="prefix" select="'c-'"/>
        <xsl:with-param name="data" select="$c"/>
      </xsl:call-template>
      <xsl:call-template name="eachLetterReverse">
